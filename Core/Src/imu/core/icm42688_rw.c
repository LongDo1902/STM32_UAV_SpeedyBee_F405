...
ICM42688_CS_Low(ICM42688_Handle_t *handle)
{
	...
	HAL_GPIO_WritePin(handle->spi_config.cs_port,
				handle->spi_config.cs_pin,
				GPIO_PIN_RESET);
}
...
ICM42688_WriteBankAuto(ICM42688_Handle_t *handle, ICM42688_Reg_t encodedReg)
{
	...
	HAL_StatusTypeDef status = HAL_SPI_Transmit(	handle->spi_config.hspi,
									bank_tx,
									2,
									ICM42688_SPI_TIMEOUT_MS);
...
{
	if ((!handle) || (!handle->spi_config.hspi) || (!handle->spi_config.cs_port)) return HAL_ERROR;

	HAL_StatusTypeDef status = ICM42688_WriteBankAuto(	handle,
										encodedReg);
	...
	uint8_t tx[2] = { (uint8_t) (regAddr & ICM42688_SPI_ADDR_MASK), val };

	ICM42688_CS_Low(handle);
	status = HAL_SPI_Transmit(	handle->spi_config.hspi,
						tx,
						2,
						ICM42688_SPI_TIMEOUT_MS);
	ICM42688_CS_High(handle);
...
{
	if ((!handle) || (!outVal) || (!handle->spi_config.hspi) || (!handle->spi_config.cs_port)) return HAL_ERROR;
	 ...
	 * 2nd sent is data byte */
	uint8_t regAddr = ICM42688_REG_ADDR(encodedReg);
	uint8_t tx[2] = { (uint8_t) ((regAddr & ICM42688_SPI_ADDR_MASK) | ICM42688_SPI_READ_BIT),
						0x00U //Dummy to clock out data
	};
	uint8_t rx[2] = { 0 };

	ICM42688_CS_Low(handle);
	status = HAL_SPI_TransmitReceive(	handle->spi_config.hspi,
							tx,
							rx,
							2,
							ICM42688_SPI_TIMEOUT_MS);
			...
			uint16_t bufLength)
{
	...
	status = HAL_SPI_Transmit(	handle->spi_config.hspi,
						...
						ICM42688_SPI_TIMEOUT_MS);
	if (status == HAL_OK) {
		status = HAL_SPI_Receive(	handle->spi_config.hspi,
							buf,
							bufLength,
							ICM42688_SPI_TIMEOUT_MS);
	}
					...
					uint8_t valueMasked)
{
	...
	return ICM42688_WriteReg(	handle,
						encodedReg,
						cur_reg);
}
