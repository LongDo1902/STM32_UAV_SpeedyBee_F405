 ...
 * @param	handle		Pointer to ICM42688 Handle struct
 * @param	encodedReg		Encoded register containing both bank number and register
 * address
 ...
 * @param	buf					Pointer to the buffer that stores the
 * received bytes
                  ...
                  uint16_t bufLength)
{
    ...
    if (((uint16_t)regAddr + bufLength) > 0x80U)
        return HAL_ERROR;
