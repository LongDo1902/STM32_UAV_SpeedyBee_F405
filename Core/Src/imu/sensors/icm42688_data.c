...
ICM42688_Get_Accel_G(ICM42688_Handle_t *handle, float g[3])
{
	...
	int16_t raw[3] = { 0 };
	HAL_StatusTypeDef status = ICM42688_Get_Accel_XYZ(	handle,
										raw);
	if (status != HAL_OK) return status;
...
ICM42688_Get_Gyro_DPS(ICM42688_Handle_t *handle, float dps[3])
{
	...
	int16_t raw[3] = { 0 };
	HAL_StatusTypeDef status = ICM42688_Get_Gyro_XYZ(	handle,
										raw);
	if (status != HAL_OK) return status;
						...
						ICM42688_Temp_Accel_Gyro_Scaled_t *sample_out)
{
	...
	int16_t raw[7] = { 0 };
	HAL_StatusTypeDef status = ICM42688_Get_Temp_Accel_Gyro_Raw(handle,
											raw);
	if (status != HAL_OK) return status;
