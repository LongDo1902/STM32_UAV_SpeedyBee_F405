                           ...
                           uint32_t samples)
{
    ...
    int32_t _sumAccelX = {0}, _sumAccelY = {0}, _sumAccelZ = {0};
    int32_t _sumGyroX = {0}, _sumGyroY = {0}, _sumGyroZ = {0};

    for (size_t i = 0; i < samples; i++) {
        (void)ICM42688_Get_Temp_Accel_Gyro_Raw(handle, &_raw);

        _sumAccelX += _raw.raw_accel[0]; // Accel X
        _sumAccelY += _raw.raw_accel[1]; // Accel Y
        _sumAccelZ += _raw.raw_accel[2]; // Accel Z

        _sumGyroX += _raw.raw_gyro[0]; // Gyro X
        _sumGyroY += _raw.raw_gyro[1]; // Gyro Y
        _sumGyroZ += _raw.raw_gyro[2]; // Gyro Z
    }

    // Save the offsets
    offsetCalibratedRaw->offset_raw_accel[0] = (int32_t)(_sumAccelX / (int32_t)samples);
    offsetCalibratedRaw->offset_raw_accel[1] = (int32_t)(_sumAccelY / (int32_t)samples);
    offsetCalibratedRaw->offset_raw_accel[2] = (int32_t)(_sumAccelZ / (int32_t)samples) -
                                               (int32_t)(lroundf(handle->accel_lsb_per_g_dtsheet));

    offsetCalibratedRaw->offset_raw_gyro[0] = (int32_t)(_sumGyroX / (int32_t)samples);
    offsetCalibratedRaw->offset_raw_gyro[1] = (int32_t)(_sumGyroY / (int32_t)samples);
    offsetCalibratedRaw->offset_raw_gyro[2] = (int32_t)(_sumGyroZ / (int32_t)samples);
                                    ...
                                    ICM42688_Temp_Accel_Gyro_Scaled_t *sampleOut)
{
    ...
    // Accel in g
    sampleOut->accel_g[0] =
        (float)((_raw.raw_accel[0] - offsetRaw->offset_raw_accel[0]) * _accel_s);
    sampleOut->accel_g[1] =
        (float)((_raw.raw_accel[1] - offsetRaw->offset_raw_accel[1]) * _accel_s);
    sampleOut->accel_g[2] =
        (float)((_raw.raw_accel[2] - offsetRaw->offset_raw_accel[2]) * _accel_s);
                                  ...
                                  ICM42688_Est_Angle_complement_t *attitudeOut, float dt_s)
{
    ...
    float _accel_x = scaledData->accel_g[0];
    float _accel_y = scaledData->accel_g[1];
    float _accel_z = scaledData->accel_g[2];

    float _gyro_x = scaledData->gyro_dps[0];
    float _gyro_y = scaledData->gyro_dps[1];
    float _gyro_z = scaledData->gyro_dps[2];
    ...
    attitudeOut->roll =
        (_alpha * (attitudeOut->roll + _gyro_x * dt_s)) + ((1.0f - _alpha) * _roll_acc);
    attitudeOut->pitch =
        (_alpha * (attitudeOut->pitch + _gyro_y * dt_s)) + ((1.0f - _alpha) * _pitch_acc);
    attitudeOut->yaw = attitudeOut->yaw + _gyro_z * dt_s;
