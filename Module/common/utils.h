...
// Expand all arguments and call macro with them. When expansion of some argument contains ',', it
// will be passed as multiple arguments #define TAKE3(_1,_2,_3) CONCAT3(_1,_2,_3) #define MULTI2 A,B
...
#else
// non ISO variant from linux kernel; checks ptr type, but triggers 'ISO C forbids braced-groups
...
cmp16(uint16_t a, uint16_t b)
{
    return (int16_t)(a - b);
}
static inline int32_t
cmp32(uint32_t a, uint32_t b)
{
    return (int32_t)(a - b);
}
...
// using memcpy_fn will force memcpy function call, instead of inlining it. In most cases function
// call takes fewer instructions
...
// Call memcpy when building unittest - this is easier that asm symbol name mangling (symbols start
// with _underscore on win32)
#include <string.h>
static inline void
memcpy_fn(void *destination, const void *source, size_t num)
{
