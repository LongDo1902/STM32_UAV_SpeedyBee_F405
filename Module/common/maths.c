...
static inline void
sincosf_quadrant_r(float r, int q, float *out_s, float *out_c)
{
    q &= 3;
...
void
sincosf_approx(float x, float *out_s, float *out_c)
{
    float t  = x * INV_PIO2;
    float qf = roundf(t);
    int   q  = (int)qf;
...
#define atanPolyCoef7 0.6444640676891548f
    ...
    if (res)
        ...
        res = 0.0f;
    res = -((((atanPolyCoef5 * res - atanPolyCoef4) * res - atanPolyCoef3) * res - atanPolyCoef2) *
                res -
            atanPolyCoef1) /
...
int32_t
applyDeadband(const int32_t value, const int32_t deadband)
{
...
float
fapplyDeadband(const float value, const float deadband)
{
...
float
devVariance(stdev_t *dev)
{
    return ((dev->m_n > 1) ? dev->m_newS / (dev->m_n - 1) : 0.0f);
}

float
devStandardDeviation(stdev_t *dev)
{
    return sqrtf(devVariance(dev));
}

float
degreesToRadians(int16_t degrees)
{
    return degrees * RAD;
}

int
scaleRange(int x, int srcFrom, int srcTo, int destFrom, int destTo)
{
...
float
scaleRangef(float x, float srcFrom, float srcTo, float destFrom, float destTo)
{
...
int32_t
quickMedianFilter3(const int32_t *v)
{
    int32_t p[3];
    QMF_COPY(p, v, 3);
...
int32_t
quickMedianFilter5(const int32_t *v)
{
    int32_t p[5];
    QMF_COPY(p, v, 5);
...
int32_t
quickMedianFilter7(const int32_t *v)
{
    int32_t p[7];
    QMF_COPY(p, v, 7);
...
int32_t
quickMedianFilter9(const int32_t *v)
{
    int32_t p[9];
    QMF_COPY(p, v, 9);
...
float
quickMedianFilter3f(const float *v)
{
    float p[3];
    QMF_COPY(p, v, 3);
...
float
quickMedianFilter5f(const float *v)
{
    float p[5];
    QMF_COPY(p, v, 5);
...
float
quickMedianFilter7f(const float *v)
{
    float p[7];
    QMF_COPY(p, v, 7);
...
float
quickMedianFilter9f(const float *v)
{
    float p[9];
    QMF_COPY(p, v, 9);
...
void
arraySubInt32(int32_t *dest, const int32_t *array1, const int32_t *array2, int count)
{
...
int16_t
qMultiply(fix12_t q, int16_t input)
{
    return (input * q) >> 12;
}

fix12_t
qConstruct(int16_t num, int16_t den)
{
    return (num << 12) / den;
}
...
float
smoothStepUpTransition(const float x, const float center, const float width)
{
