...
static inline void
__basepriRestoreMem(uint8_t *val)
{
    atomic_BASEPRI = *val;
...
static inline uint8_t
__basepriSetMemRetVal(uint8_t prio)
{
...
static inline uint8_t
__basepriSetRetVal(uint8_t prio)
{
    if (prio && (atomic_BASEPRI == 0 || atomic_BASEPRI > prio)) {
        atomic_BASEPRI = prio;
    }
...
static inline void
__do_cleanup(__cleanup_block *b)
{
    (*b)();
}
