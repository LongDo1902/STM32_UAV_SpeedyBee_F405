 ...
 *        The encoded bank is selected automatically before the blocking SPI transfer, and the handle's cached
 *        bank is updated only when bank selection succeeds.
 ...
 *        The encoded bank is selected automatically and the received byte is stored only through the caller's
 *        output pointer, which must remain valid for the complete blocking transaction.
 * @param pHandle Pointer to an initialized ICM42688 handle.
 * @param encodedReg Register encoded with ICM42688_REG(), including bank and address.
 ...
 *        The function prepares the command and dummy TX bytes, asserts CS, and returns as soon as HAL accepts
 * the asynchronous transfer. Both DMA buffers must remain unchanged until the completion/error callback.
 * @param pHandle Pointer to an initialized ICM42688 handle.
 * @param startEncodedReg First encoded register to read.
 ...
 * asynchronous transfer. The caller owns completion handling and must keep pTxBuf valid until DMA finishes.
 * @param pHandle Pointer to an initialized ICM42688 handle.
 * @param encodedReg Register encoded with ICM42688_REG(), including bank and address.
 * @param data Byte to write.
