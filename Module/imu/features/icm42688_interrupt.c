...
ICM42688_Int_Status_Has(uint8_t status, ICM42688_Int_Status_t intState)
{
    ...
    switch (intState) {
        case INT_AGC_RDY:
            _mask = (uint8_t)ICM42688_AGC_RDY_INT_Msk;
            break;
        case INT_FIFO_FULL:
            _mask = (uint8_t)ICM42688_FIFO_FULL_INT_Msk;
            break;
        case INT_FIFO_THS:
            _mask = (uint8_t)ICM42688_FIFO_THS_INT_Msk;
            break;
        case INT_DATA_RDY:
            _mask = (uint8_t)ICM42688_DATA_RDY_INT_Msk;
            break;
        case INT_RESET_DONE:
            _mask = (uint8_t)ICM42688_RESET_DONE_INT_Msk;
            break;
        case INT_PLL_RDY:
            _mask = (uint8_t)ICM42688_PLL_RDY_INT_Msk;
            break;
        case INT_UI_FSYNC:
            _mask = (uint8_t)ICM42688_UI_FSYNC_INT_Msk;
            break;
        default:
            return false;
    }

    return (((status & 0x7FU) & _mask) != 0U);
}
 ...
 *==========================================================================================*/
bool
ICM42688_Set_INT_CONFIG1(ICM42688_Handle_t *pHandle, bool setAsyncReset, bool setTdeAssertDisable,
                         bool setTpulseDuration)
{
    ...
    uint8_t _mask =
        ICM42688_INT_ASYNC_RESET_Msk | ICM42688_INT_TDEASSERT_DISABLE_Msk | ICM42688_INT_TPULSE_DURATION_Msk;
    uint8_t _value_masked = ICM42688_INT_ASYNC_RESET_Val((uint8_t)setAsyncReset) |
                            ICM42688_INT_TDEASSERT_DISABLE_Val((uint8_t)setTdeAssertDisable) |
                            ICM42688_INT_TPULSE_DURATION_Val((uint8_t)(setTpulseDuration));

    bool _status = ICM42688_Update_Reg_Bits(pHandle, ICM42688_UB0_INT_CONF1, _mask, _value_masked);
    if (!_status)
        return _status;
